...
:: ===================================================
set "APP_NAME=OTP Manager"
set "APP_EXE=otp-manager.exe"  :: <-- Nama file executable aplikasi Anda
set "REPO_URL=https://github.com/Sugandi1994/TELEGRAM-DOWNLOADER-SCRIPT-VIOLENT-MONKEY/archive/refs/heads/main.zip"
...
powershell -Command "try { Invoke-WebRequest -Uri '%REPO_URL%' -OutFile '%ZIP_FILE%' } catch { Write-Host 'ERROR: Gagal mengunduh file.'; exit 1 }"
if %errorLevel% neq 0 (
...
xcopy "%EXTRACT_DIR%\otp-manager-ope-main\*" "." /E /Y /I /Q /EXCLUDE:exclude_update.txt
if %errorLevel% neq 0 (
